...
class TestLinearOperator(TestCase):
    def setUp(self):
        ...
        def rmatvec2(x):
            return np.matrix(rmatvec1(x).reshape(3, 1))

        self.matvecs.append(matvec1)
        self.matvecs.append(matvec2)
        self.rmatvecs.append(rmatvec1)
        self.rmatvecs.append(rmatvec2)

    def test_matvec(self):

        for matvec, rmatvec in zip(self.matvecs, self.rmatvecs):
            A = LinearOperator((2,3), matvec, rmatvec=rmatvec)
            ...
            assert_equal((A + 1) * np.array([1, 2, 3]), [15, 34])
            ...
            assert( isinstance(A.T, LinearOperator) )
            assert( isinstance(A.T * A, LinearOperator) )
